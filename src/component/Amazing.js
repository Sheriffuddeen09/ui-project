const Amazing = () =>{


    return(
        <div className="h055">
        <p className="h056">
            See what <span className="h057"> others are saying.</span>
        </p>
        <span className="h058"></span>

        <div className="h059">
            <div className="h060">
                <p className="h061">
                    “Amazing team to work with.”
                </p>
                <p className="h062">
                    Their team is knowledgeable and always goes above and beyond to
                    <br/> ensure my shipments arrive on time and in perfect condition.
                </p>
            </div>
            <div className="h063">
                <p className="h064">
                   <span className="h066"> Lanre Sodeeq</span>
                    <br/>
                    Experienced Agro Exporter
                    <br/>
                    2/3/23
                </p>

            </div>
        </div>
        <span className="h067"></span>
        <div className="h059">
            <div className="h060">
                <p className="h061">
                    “Fast and effective”
                </p>
                <p className="h062">
                    Oneport365 has truly transformed the way I approach shipping. Their 
                    <br/>
                    innovative technology and streamlined processes have saved me time and 
                    <br/>
                    money.
                </p>
            </div>
            <div className="h063">
                <p className="h064">
                    <span className="h066"> Raj Kamapoosha</span>
                     <br/>
                     Manager at Chatra 
                     <br/>
                     Industries ltd.
                     <br/>
                     1/3/23
                 </p>

            </div>
        </div>
        <span className="h067"></span>
        <div className="h059">
            <div className="h060">
                <p className="h061">
                    “Was easier than I expected”
                </p>
                <p className="h062">
                    Their team is knowledgeable and always goes above and beyond to 
                    <br/>
                    ensure my shipments arrive on time and in perfect condition.
                </p>
            </div>
            <div className="h063">
                <p className="h064">
                   <span className="h066"> David Okon</span>
                    <br/>
                    Major dealer at Odun-Ade
                    <br/>
                    5/4/23
                </p>

            </div>
        </div>
        
    </div>



    )
}
export default Amazing